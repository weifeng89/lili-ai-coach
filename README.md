...
│   ├── eval-criteria.md           # AC1–AC14(用户能力AC1–10/打包质量AC11–14) + UC1–UC20
│   ├── eval-checklist.json
│   └── eval-checklist.csv
...
## 验收 / 质量
交付标准见 `eval/eval-criteria.md`（AC1–AC14 + UC1–UC20，参考 [mattpocock/skills](https://github.com/mattpocock/skills) 的「小且可组合 / 文档中立 / 版本同步」质量杆，改为非技术初学者口径）。机读勾选版：`eval/eval-checklist.json` / `.csv`。验收分两段：AC1–AC10 为用户可见能力，AC11–AC14 为 mentor 专用质量闸（用户不可见）。

重点验收项：
- **AC11 跨 harness 一致**：`SKILL.md` 与 `agents/openai.yaml` 方法论正文逐字符一致。
- **AC8 零安装感知**：lili酱 端零文件操作；install 脚本落盘到正确目录、版本校验、保留 log、不拷 eval/examples。
- **AC9 术语零暴露**：面向 lili酱 的输出无内部术语，方法论均说人话。
- **AC7 Token 优化**：自纪律（压缩优先 / 懒加载 / 产物落文件 / 软预算）+ 用户侧模式级诊断（自动触发下仍控上下文，不谎称读真实 token）。

---

## 风险与边界（给 mentor）
- **R9 自动触发双触发 / 双脑困惑**：查理自动触发与 base Agent 自身能力可能重叠，小白易分不清「现在是谁在答」。缓解：`description` 精准限定「AI 协作/沟通/需求/PRD/workflow/卡点/提效」；纯领域问题走柔性分流产出信封交 base，不自答（见 SKILL.md 🚦）；**查理切入时轻量身份提示**（本轮首次接手报「我是查理👋」，不每轮刷），消除用户感知层困惑（见 SKILL.md 🚦「切入时轻量身份提示」）。
- **R10 安装脚本依赖**：需网络 / git 权限；无网环境退化到手动拷贝白名单（见上方「下载白名单」）。`npx add-skill` 仅 WorkBuddy 新版可用，Hermes/Codex 请用 `install.py`。
- **R11 柔性分流靠模型判断力**：「识别是否纯领域问题、要不要分流」本身依赖模型软判断，弱模型可能该分流不分流、或不该分流乱分流。缓解：把硬分类改为**输出末尾永远附「可直接发的精准指令」**的形态约束，降低对模型判断的依赖；残差风险接受，不为此加复杂规则（所有 Skill 项目的共性难题，非本项目独有）。
